...
export const Route = createFileRoute("/products/$slug")({
  head: ({ params }) => {
    ...
    const desc =
      seo?.description ??
      cat?.blurb ??
      "Explore our premium products at Durga Hardware and Plywood.";
  ...
  return (
    <PageLayout>
        ...
        <section className="py-[30px] bg-cream">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 grid gap-12 lg:grid-cols-2 lg:gap-16">
            <div>
              ...
              <h3 className="mt-10 font-display text-xl font-bold text-charcoal">
                Why It Stands Out
              </h3>
              <ul className="mt-5 grid gap-3">
                {detail.highlights.map((h) => (
      ...
      <section className="py-[30px] bg-cream">
        ...
        {slug === "plywood" &&
          PLYWOOD_BRANDS.map((brand, bIdx) => (
            <section key={brand.name} className="py-[30px] bg-cream">
              <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                <div className="text-center max-w-3xl mx-auto">
                  <span className="eyebrow">Featured Brand</span>
                  <h2 className="mt-4 font-display text-3xl sm:text-4xl font-bold text-charcoal">
        ...
        {slug === "laminates" && (
          <BrandMarquee
            title="Laminate Brands"
            logos={LAMINATE_BRAND_LOGOS}
            className="py-[30px]"
          />
        )}
